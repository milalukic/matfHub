...
Course KmiljanScraper::tdToCourse(QString tdHtml, int day, int start)
{
  ...
  QRegularExpression tdRegex(
      ">([^<]*)<"); // ovo radi sad jer smo radili replace
  ...
  std::map<std::string, std::string> moduleMap = {
      {"I", "Informatika"}, {"R", "Racunarstvo"}, {"M", "Teorijska"},
      {"N", "Primenjena"},  {"V", "Aktuarska"},   {"L", "Profesor"}};
  for (auto sg : qgroups) {
    sg = sg.replace(QString::fromStdString("O"),
                    QString::fromStdString("RMNVL"));
  ...
  return Course(name.toStdString(), day, staff.toStdString(), start - 8, length,
                lectureType.toStdString(), groups, placeholderClassroom,
                modules, years, subgroups);
}
...
CourseSet KmiljanScraper::tableToCourses(QString tableHtml,
                                         std::string classroom)
{
  ...
  for (auto &line : lines) {
    //        if(line.contains("Превођење")){                        //Ovo je
    //        vrv neko debugovanje bilo, printa ceo html
    //            std::cout << 3;
    //        }
...
CourseSet KmiljanScraper::getAllCourses(
    std::unordered_map<std::string, std::string> roomMap) noexcept(false)
{
  ...
  for (const auto &room : roomMap) {
    ...
    QString htmlString =
        QString::fromStdString(requestHandler.getHtml(roomLink));
    QRegularExpression tableRegex(
        R"(<table border="1" bordercolor="#000000" cellspacing="0" cellpadding="0">(.*)</table>)",
...
std::unordered_map<std::string, CourseSet>
KmiljanScraper::download() noexcept(false)
{
