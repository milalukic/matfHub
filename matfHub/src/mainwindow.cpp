      ...
      ui(new Ui::MainWindow)
{
  ...
  QString schedulePath =
      Config::getConfig()->getConfigPath() + "/raspored.json";
  QFile scheduleFile(schedulePath);
  if (scheduleFile.exists() &&
      scheduleFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
  ...
  connect(saveShortcut, &QShortcut::activated, this,
          &MainWindow::on_saveNotesButton_clicked);
  ...
  connect(openShortcut, &QShortcut::activated, this,
          &MainWindow::on_openFileNotesButton_clicked);
  ...
  connect(newShortcut, &QShortcut::activated, this,
          &MainWindow::on_newFileNotesButton_clicked);
  ...
  connect(ui->dirView, &QTreeView::doubleClicked, this,
          &MainWindow::dirViewDoubleClicked);

  connect(ui->backButton, &QPushButton::clicked, this,
          &MainWindow::backButtonClicked);
  ...
  connect(ui->homeButton, &QPushButton::clicked, this,
          &MainWindow::homeButtonClicked);
  connect(ui->dotDotButton, &QPushButton::clicked, this,
          &MainWindow::dotDotButtonClicked);
  connect(ui->newFolderButton, &QPushButton::clicked, this,
          &MainWindow::newFolderButtonClicked);

  connect(ui->currentFilePath, &QLineEdit::editingFinished, this,
          &MainWindow::currentFilePathEditingFinished);

  setUpFileView();

  // TODO mozda neki connectCalc(this) koji ovo sve zove da bi konstruktor bio
  // cistiji? slicno za ostale konekcije
  ...
  connect(ui->pbChangeRegular, &QPushButton::clicked, this,
          &MainWindow::changeStackedWidgetPage);
  connect(ui->pbChangeMatrix, &QPushButton::clicked, this,
          &MainWindow::changeStackedWidgetPage);
  connect(ui->pbChangePlot, &QPushButton::clicked, this,
          &MainWindow::changeStackedWidgetPage);
  connect(ui->pbChangeStat, &QPushButton::clicked, this,
          &MainWindow::changeStackedWidgetPage);

  connect(ui->pbParser, &QPushButton::clicked, this,
          &MainWindow::calculateRegular);

  connect(ui->pbMatrixTranspose, &QPushButton::clicked, this,
          &MainWindow::calculateMatrixTranspose);
  connect(ui->pbMatrixInverse, &QPushButton::clicked, this,
          &MainWindow::calculateMatrixInverse);
  connect(ui->pbMatrixEye, &QPushButton::clicked, this,
          &MainWindow::calculateMatrixEye);
  connect(ui->pbMatrixOne, &QPushButton::clicked, this,
          &MainWindow::calculateMatrixOne);

  connect(ui->pbMatrixLoad1, &QPushButton::clicked, this,
          &MainWindow::reshapeMatrix1);
  connect(ui->pbMatrixLoad2, &QPushButton::clicked, this,
          &MainWindow::reshapeMatrix2);

  connect(ui->pbMatrixAdd, &QPushButton::clicked, this,
          &MainWindow::calculateMatrixAdd);
  connect(ui->pbMatrixSubtract, &QPushButton::clicked, this,
          &MainWindow::calculateMatrixSubtract);
  connect(ui->pbMatrixMultiply, &QPushButton::clicked, this,
          &MainWindow::calculateMatrixMultiply);
  connect(ui->pbMatrixDiagonal, &QPushButton::clicked, this,
          &MainWindow::calculateMatrixDiag);

  // connect(ui->listFileView, &QPushButton::clicked, this,
  // &MainWindow::on_fileView_doubleClicked);

  // kalk

  connect(ui->pbSaveMatrix, &QPushButton::clicked, this,
          &MainWindow::saveMatrix);
  ...
  connect(ui->pbLinspace, &QPushButton::clicked, this,
          &MainWindow::plotLinspace);
  connect(ui->pbPlotSave, &QPushButton::clicked, this,
          &MainWindow::savePlotting);

  connect(ui->pbMean, &QPushButton::clicked, this, &MainWindow::statCalcMean);
  connect(ui->pbVariance, &QPushButton::clicked, this,
          &MainWindow::statCalcVariance);
  connect(ui->pbStd, &QPushButton::clicked, this, &MainWindow::statCalcStd);
  connect(ui->pbMedian, &QPushButton::clicked, this,
          &MainWindow::statCalcMedian);
  ...
  connect(ui->pbSwitchMatrices, &QPushButton::clicked, this,
          &MainWindow::switchMatrices);

  connect(ui->pbHistorySave, &QPushButton::clicked, this,
          &MainWindow::historySave);

  connect(ui->textEdit, &QTextEdit::textChanged, this,
          [this]() { m_notes->notesContentChanged(this, ui); });
}
...
void MainWindow::
    on_saveNotesButton_clicked() // save/save as? trenutno najlaksa opcija da se
                                 // sacuva izmena jednog fajla u drugi je ctrl+a
                                 // ctrl+c ctrl+n ctrl+v ctrl+s (takodje bilo bi
                                 // kul da se prva tri dugmeta aktiviraju i na
                                 // ctrl+n ctrl+s i ctrl+o
{
  if (ui->tabWidgetMatfHub->currentIndex() == 2) {
...
void MainWindow::backButtonClicked()
{
  ...
  ui->detailsFileView->horizontalHeader()->resizeSections(
...
void MainWindow::fileViewCustomContextMenuRequested(
    const QPoint &pos,
    QAbstractItemView
        *view) // TODO !!!!!! ovo bi bilo lepo takodje izmestiti u
               // filemanager.cpp ali prvo connect je iz QObject ili tako necega
               // treba to nasledjivanje srediti drugo poslednja linija funkcije
               // (i jos neke) zove privatne metode privatnih polja iz ui pa bi
               // i to trebalo prebroditi nekako nekim seterom
{ // TODO pored komentara u prethodnoj liniji takodje ova f-ja postaje glomazna,
  // bilo bi lepo mozda razdvojiti je u pozive f-ja koje pojedinacno prave
  // akcije
  ...
  QAction *list = new QAction(
      "List", this); // ovo mozda ima smisla smestiti u petlju samo bi onda
                     // naziv akcije morao nekako da se dobije od parsiranja
                     // naziva ovih pogleda TOTO?
  viewSubMenu->addAction(list);
  connect(list, &QAction::triggered, this,
          [this]() { MainWindow::showFileView(ui->listFileView); });
  QAction *table = new QAction("Details", this);
  viewSubMenu->addAction(table);
  connect(table, &QAction::triggered, this,
          [this]() { MainWindow::showFileView(ui->detailsFileView); });
  QAction *thumbnails = new QAction("Thumbnails", this);
  viewSubMenu->addAction(thumbnails);
  connect(thumbnails, &QAction::triggered, this,
          [this]() { MainWindow::showFileView(ui->thumbnailsFileView); });
  ...
  if (noSelected == 1) {
    ...
    connect(renameAction, &QAction::triggered, this, [=]() {
      ...
      QString newName = QInputDialog::getText(
          this, "Preimenuj fajl", "Nov Naziv:", QLineEdit::Normal, oldName);
  ...
  if (noSelected > 0) {
    ...
    connect(deleteAction, &QAction::triggered, this, [=]() {
      QModelIndexList selectedIndices = getSelectedIndices(
          view); // meni se indices vise svidja kao mnozina od index nego
                 // indexes ali Bojane slobodno promeni ako zelis nije mi bitno
...
void MainWindow::showFileView(QAbstractItemView *view)
{
  ...
  for (int i = 0; i < ui->fileViewLayout->count(); ++i) {
    QAbstractItemView *widget = static_cast<QAbstractItemView *>(
        ui->fileViewLayout->itemAt(i)->widget());
    widget->hide();
  }
  view->show();
}
// naredne dve funkcije mozda treba pomeriti u filemanager klasu ali mislim da
// je to pametnije uraditi kada krenemo da spajamo grane u zavisnosti od toga
// sta odlucimo kako ce se ponasati meni bar glavnog prozora
...
{
  QString newHubPath =
      QFileDialog::getExistingDirectory(this, "Odaberi direktorijum");
  ...
  ui->detailsFileView->setRootIndex(
      m_fileManager->m_fileModel->setRootPath(m_fileManager->m_currPath));
}
...
{
  for (int i = 0; i < ui->fileViewLayout->count(); ++i) {
    auto view = static_cast<QAbstractItemView *>(
        ui->fileViewLayout->itemAt(i)->widget());
    ...
    if (QTableView *table = dynamic_cast<QTableView *>(view)) {
      table->horizontalHeader()->resizeSections(
          QHeaderView::ResizeToContents); // TODO iz nekog razloga cini mi se da
                                          // ovo ne radi UVEK ako se prvi put
                                          // ulazi u neki folder?
    }
  }
}
...
void MainWindow::setUpFileView(/*tipPogleda*/)
{
  ...
  connect(
      ui->detailsFileView, &QTableView::doubleClicked, this,
      [this]() { // TODO koji signal je odgovarajuc? nesto sto odgovara promeni
                 // m_currPath i nista drugo? komplikovan nacin bi bio currPath
                 // retvoriti u klasu koja pored stringa ima i signal koji se
                 // ovde salje pri promeni QStringa ali to me smara trenutno...
        ui->detailsFileView->horizontalHeader()->resizeSections(
            QHeaderView::ResizeToContents);
      });
  ...
  for (int i = 0; i < ui->fileViewLayout->count(); ++i) {
    auto widget = ui->fileViewLayout->itemAt(i)->widget();
    QAbstractItemView *view = static_cast<QAbstractItemView *>(
        widget); // auto ga vec kastuje u konkretan tip dakle QTableView,
                 // QListView itd B) ali kompajler u daljem kodu to ne zna nego
                 // je u fazonu ok ovo ce sigurno biti bar QObject tako da ipak
                 // mora da se kastuje sto je idiotski ali tako je D: jednom
                 // kada naprave funkcionalne procesore koji lenjo izracunavaju
                 // i kada ceo svet predje na normalne funkcionalne
                 // interpretirane jezike ovo nece biti problem ;)
    // qDebug() << view;//apdejt: i ovaj staticki kast ga kastuje u klasu koja
    // treba da bude, nagadjam jer je AbstractView apstraktna klasa TODO nauci
    // vise o cppu

    view->setModel(m_fileManager->m_fileModel);
    view->setRootIndex(
        m_fileManager->m_fileModel->setRootPath(m_fileManager->m_hubPath));
    view->setSelectionMode(QAbstractItemView::ExtendedSelection);

    // connect(ui->detailsFileView, &QTableView::doubleClicked, m_fileManager,
    // &FileManager::fileViewDoubleClicked); ovo ne radi a zelim da proradi,
    // TODO istraziti
    connect(view, &QAbstractItemView::doubleClicked, this,
            &MainWindow::fileViewDoubleClicked);
    connect(view, &QAbstractItemView::customContextMenuRequested, this,
            [this, view](QPoint pos) {
  ...
  // naredna linija zakomentarisana za lakse dibagovanje, TODO obrisati je kad
  // tad
...
void MainWindow::reshapeMatrix(unsigned dim1, unsigned dim2, unsigned pos)
{
  ...
  pos == 1 ? m1->reshapeMatrix(realDim2, realDim1)
           : m2->reshapeMatrix(realDim2, realDim1);

  QVBoxLayout *rows;
  QWidget *scrollAreaWidget =
      (pos == 1 ? ui->scrollAreaM1widget : ui->scrollAreaM2widget);
  ...
  QStringList content =
      matrixStringToStringList(pos == 1 ? (m1->toString()) : (m2->toString()));
...
void MainWindow::reshapeMatrix(unsigned dim1, unsigned dim2, unsigned pos,
                               QStringList content)
{ // TODO napraviti enum za pos LEFT i RIGHT
...
void MainWindow::calculateMatrixTranspose()
{
  ...
  history->writeHistory("Transponovanje matrice:",
                        m3->toString().toStdString());
  writeToHistoryTB(history);
}
...
void MainWindow::saveMatrix()
{
  ...
  connect(loadLeft, &QPushButton::clicked, this, [this, index]() {
    ...
    loadMatrix(1, matrixStringToStringList(Matrix::getSaved(index)->toString()),
               d1, d2);
  });
  connect(loadRight, &QPushButton::clicked, this, [this, index]() {
    auto [d2, d1] = m3->loadRight(index);
    loadMatrix(2, matrixStringToStringList(Matrix::getSaved(index)->toString()),
               d1, d2);
  });
}

void MainWindow::loadMatrix(unsigned int pos, QStringList strLst, unsigned d1,
                            unsigned d2)
{
