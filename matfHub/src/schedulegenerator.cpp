...
bool Generator::scheduleFilter(const std::vector<Course> &schedule)
{
  ...
  auto classroomGroup = [](const Course &course) -> int {
    if (course.classroom.empty())
      return 0;
    if (course.classroom[0] == 'J')
      return 1;
    if (course.classroom[0] == 'N')
      return 2;
    return 0;
  };
  ...
  std::sort(schedules.begin(), schedules.end(),
            [](const auto &lhs, const auto &rhs) {
              return std::accumulate(lhs.begin(), lhs.end(), 0,
                                     [](int sum, const Course &course) {
                                       return sum + course.start;
                                     }) <
                     std::accumulate(
                         rhs.begin(), rhs.end(), 0,
                         [](int sum, const Course &course) {
...
void Generator::displaySchedule(QTableWidget *tableWidget, int brojRasporeda)
{
  ...
  if (!schedules.empty()) {
    ...
    for (auto &course : schedule) {
      ...
      int end = course.end;
      QTableWidgetItem *subItem = new QTableWidgetItem(QString::fromStdString(
          course.description + '\n' + course.course_type + '\n' +
          course.teacher + '\n' + course.classroom));
      subItem->setBackground(
          QBrush(getPastelColor(QString::fromStdString(course.description))));
...
void Generator::_find(StrMap<StrMap<CourseSet>> &courseTypeTermMap,
                      std::vector<Course> &placedList, int i, int stop,
                      std::unordered_set<std::string> alreadySearched)
{
  if (i == stop) {
  ...
  for (const auto &pair : courseTypeTermMap) {
    ...
    for (const auto &pair : typeTermMap) {
      ...
      for (const auto &term : terms) {
        bool notPlacedYet =
            placed.find(term.description + term.course_type) == placed.end();
