...
void FileManager::createNewFolder()
{
  ...
  while (QDir(newFolderPath).exists()) {
    int foundSpace =
        name.lastIndexOf("r") + 1; // osecam se kao da je ovo r magicna
                                   // konstanta nz treba li je promeniti
...
// prebacuje desni pogled u zadati direktorijum, postavlja oldPath i currPath,
// na pozivaocu funkcije je da promenu zabelezi u istoriji navigacije
...
void FileManager::fileViewDoubleClicked(const QModelIndex &index)
{
  if (m_fileModel->fileInfo(index).isDir()) {
    ...
    if (lastDot != -1) {
      auto fileExtension =
          clickedFilePath.last(clickedFilePath.length() - lastDot);
      if (".txt" == fileExtension || ".json" == fileExtension ||
          ".cpp" == fileExtension) {
        m_mainWindow->ui->tabWidgetMatfHub->setCurrentIndex(
            2); // ovo je tvrdo kodirano, bilo bi lepo izvuci indeks iz imena
                // ili makar definisati enum
        m_mainWindow->m_notes->openFile(clickedFilePath, m_mainWindow->ui,
                                        m_mainWindow);
      } else {
        QDesktopServices::openUrl(
            m_fileModel->fileInfo(index).absoluteFilePath());
      }
    }
  }
}
...
void FileManager::currentFilePathEditingFinished()
{
  ...
  m_mainWindow->currentFilePathSetPath(
      newPath); // ovo radimo bez obzira na poziv changeDir da bi se obrisale
                // kose crte ako su one jedine dodate
...
void FileManager::renameSelectedFile(const QModelIndex index,
...
{
  // qDebug() << "Column 0:" << fileModel->headerData(0, Qt::Horizontal,
  // Qt::DisplayRole);
...
{
  // qDebug() << "Column 3:" << fileModel->headerData(3, Qt::Horizontal,
  // Qt::DisplayRole);
...
{
  // qDebug() << "Column 1:" << fileModel->headerData(1, Qt::Horizontal,
  // Qt::DisplayRole);
...
{
  // qDebug() << "Column 2:" << fileModel->headerData(2, Qt::Horizontal,
  // Qt::DisplayRole);
